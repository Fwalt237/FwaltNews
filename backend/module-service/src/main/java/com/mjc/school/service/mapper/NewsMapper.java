...
public abstract class NewsMapper {
  ...
  @Mapping(target = "author", expression = "java(authorRepository.findByName(dto.author()).get())")
  @Mapping(
      target = "tags",
      expression =
  ...
  void setComments(News model, @MappingTarget NewsDtoResponse dto) {
    if (Hibernate.isInitialized(model.getComments()) && model.getComments() != null) {
      List<CommentsDtoForNewsResponse> commentsDto =
          model.getComments().stream().map(c -> commentMapper.modelToDtoForNews(c)).toList();
