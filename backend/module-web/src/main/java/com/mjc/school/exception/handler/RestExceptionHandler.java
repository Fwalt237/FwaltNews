...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  protected ResponseEntity<ErrorResponse> handleApiVersionNotSupportedException(
      ApiVersionNotSupportedException exc) {
  ...
  protected ResponseEntity<ErrorResponse> handleValidationException(Exception exc) {
    return buildErrorResponse(
        ...
        String.format(VALIDATION.getMessage(), exc.getMessage()),
        exc.getMessage(),
        HttpStatus.BAD_REQUEST);
  }

  @ExceptionHandler(value = NotFoundException.class)
  protected ResponseEntity<ErrorResponse> handleResourceNotFoundException(Exception exc) {
    return buildErrorResponse(
  ...
  protected ResponseEntity<ErrorResponse> handleResourceConflictException(ServiceException exc) {
    return buildErrorResponse(
        exc.getCode(), exc.getMessage(), exc.getDetails(), HttpStatus.CONFLICT);
  }

  @ExceptionHandler(value = BadCredentialsException.class)
  protected ResponseEntity<ErrorResponse> handleBadCredentialsException(
      BadCredentialsException bce) {
    return buildErrorResponse(
        BAD_CREDENTIALS.getErrorCode(),
  ...
  protected ResponseEntity<ErrorResponse> handleUsernameNotFoundException(
    ...
    return buildErrorResponse(
        USERNAME_DOES_NOT_EXIST.getErrorCode(),
        USERNAME_DOES_NOT_EXIST.getMessage(),
        unfe.getMessage(),
        HttpStatus.UNAUTHORIZED);
  }
  ...
  protected ResponseEntity<ErrorResponse> handleAiIntegrationException(AiIntegrationException aie) {
    return buildErrorResponse(
        AI_EMBEDDING_FAILED.getErrorCode(),
        AI_EMBEDDING_FAILED.getMessage(),
        aie.getMessage(),
  ...
  protected ResponseEntity<ErrorResponse> handleUnexpectedException(Exception exc) {
    return buildErrorResponse(
        UNEXPECTED_ERROR.getErrorCode(),
        UNEXPECTED_ERROR.getMessage(),
        exc.getMessage(),
        HttpStatus.INTERNAL_SERVER_ERROR);
  }

  private ResponseEntity<ErrorResponse> buildErrorResponse(
      String code, String errorMessage, String errorDetails, HttpStatus status) {
