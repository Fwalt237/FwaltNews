...
public class AuthorModelAssembler
    implements RepresentationModelAssembler<AuthorDtoResponse, AuthorDtoResponse> {
    ...
    author.add(
        Link.of(
            linkBuilder.buildResourceLinkForMethod(
                AuthorController.class, "readById", author.getId()),
            "self"));

    author.add(Link.of(linkBuilder.buildCollectionLink(AuthorController.class), "authors"));

    author.add(
        Link.of(linkBuilder.buildResourceLink(AuthorController.class, author.getId()), "update"));

    author.add(
        Link.of(linkBuilder.buildResourceLink(AuthorController.class, author.getId()), "delete"));

    return author;
  }

  @Override
  public CollectionModel<AuthorDtoResponse> toCollectionModel(
      Iterable<? extends AuthorDtoResponse> entities) {
    CollectionModel<AuthorDtoResponse> authorModels =
