...
public class CommentModelAssembler
    implements RepresentationModelAssembler<CommentsDtoResponse, CommentsDtoResponse> {
  ...
  public CommentsDtoResponse toModel(CommentsDtoResponse comment) {
    ...
    comment.add(
        Link.of(
            linkBuilder.buildResourceLinkForMethod(
                CommentController.class, "readById", comment.getId()),
            "self"));
    ...
    if (comment.getNewsId() != null) {
      comment.add(
          Link.of(
              linkBuilder.buildResourceLink(NewsController.class, comment.getNewsId()), "news"));
    }

    comment.add(
        Link.of(linkBuilder.buildResourceLink(CommentController.class, comment.getId()), "update"));

    comment.add(
        Link.of(linkBuilder.buildResourceLink(CommentController.class, comment.getId()), "delete"));

    return comment;
  }

  @Override
  public CollectionModel<CommentsDtoResponse> toCollectionModel(
      Iterable<? extends CommentsDtoResponse> entities) {
    CollectionModel<CommentsDtoResponse> commentModels =
