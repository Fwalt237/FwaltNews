...
public class PageModelAssembler {
  ...
  public <T> PageDtoResponse<T> addPaginationLinks(
      PageDtoResponse<T> page, ResourceSearchFilterRequestDTO searchRequest, String baseUrl) {
  ...
  private Link createPageLink(
      String baseUrl, ResourceSearchFilterRequestDTO searchRequest, int pageNumber) {
    ...
    if (searchRequest.getSortByAndOrder() != null && !searchRequest.getSortByAndOrder().isEmpty()) {
      searchRequest.getSortByAndOrder().forEach(sort -> builder.queryParam("sortByAndOrder", sort));
    }

    if (searchRequest.getSearchFilter() != null && !searchRequest.getSearchFilter().isEmpty()) {
      searchRequest
          .getSearchFilter()
          .forEach(filter -> builder.queryParam("searchCriteria", filter));
    }
