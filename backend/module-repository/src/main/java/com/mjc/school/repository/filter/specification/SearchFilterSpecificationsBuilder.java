...
public class SearchFilterSpecificationsBuilder<T> {
  ...
  public SearchFilterSpecificationsBuilder<T> withSearchCriteriaList(
  ...
  public SearchFilterSpecificationsBuilder<T> withSearchCriteria(
      final SearchCriteria searchCriteria) {
    criteriaList.add(searchCriteria);
    return this;
  }

  public SearchFilterSpecificationsBuilder<T> with(
  ...
  public Specification<T> build() {
    ...
    List<SearchFilterSpecification<T>> specs =
        criteriaList.stream().map(SearchFilterSpecification<T>::new).toList();

    Specification<T> result = specs.get(0);

    for (int i = 1; i < criteriaList.size(); i++) {
      result =
          criteriaList.get(i).isOrPredicate()
