...
public interface NewsRepository extends JpaRepository<News, Long>, JpaSpecificationExecutor<News> {
  ...
  @Query(
      "SELECT n FROM News n JOIN n.tags t "
          + "WHERE LOWER(t.name) = LOWER(:tagName) AND n.createdDate >= :since")
  List<News> findByTagNameAndCreatedDateAfter(
      @Param("tagName") String tagName, @Param("since") LocalDateTime since, Pageable pageable);
  ...
  @Query(
      "SELECT n FROM News n LEFT JOIN NewsEmbeddings ne ON n.id = ne.news.id WHERE ne.id IS NULL")
  List<News> findNewsWithoutEmbeddings();

  List<News> findByCreatedDateBefore(LocalDateTime tomorrow);
}
